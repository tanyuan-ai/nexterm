(function(){
  ...
  ready(function(){
    ...
    function footerClock(){
      ...
      // Custom prefix support: replace a leading "--" placeholder with the live time; otherwise append time after the prefix
      var tpl=x.getAttribute('data-template')||'CST / --';
      var timeStr=p.hour+':'+p.minute+':'+p.second;
      if(tpl.indexOf('--')>-1){x.textContent=tpl.replace('--',timeStr);}
      else{x.textContent=tpl+' '+timeStr;}
    }
    footerClock();
